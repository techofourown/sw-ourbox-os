...
## Publish record JSON outputs

Each upstream publish flow emits a canonical machine-readable publish record in `dist/`:
...
These records are validated against `schemas/artifact-publish-record.schema.json`.
