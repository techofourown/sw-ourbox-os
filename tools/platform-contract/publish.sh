...
command -v oras >/dev/null 2>&1 || { echo "oras is required (https://oras.land/)." >&2; exit 1; }
...
pushd "${ROOT}" >/dev/null
set +e
OUT="$(oras push "${REF}" --artifact-type "${ARTIFACT_TYPE}" "${BLOB_REL}:application/gzip" \
  ...
  --annotation "techofourown.artifact.kind=platform-contract" 2>&1)"
STATUS=$?
set -e
popd >/dev/null

printf '%s\n' "${OUT}" | tee "${PUSH_LOG}"
[[ "${STATUS}" -eq 0 ]] || { echo "oras push failed" >&2; exit "${STATUS}"; }

DIGEST="$(printf '%s\n' "${OUT}" | grep -Eo 'sha256:[0-9a-f]{64}' | tail -n1)"
[[ -n "${DIGEST}" ]] || { echo "Failed to capture digest" >&2; exit 1; }
...
python3 "${ROOT}/tools/publish-records/write-publish-record.py" \
  ...
  --artifact-metadata "OURBOX_PLATFORM_CONTRACT_SOURCE=${OURBOX_PLATFORM_CONTRACT_SOURCE}" \
  --artifact-metadata "OURBOX_PLATFORM_CONTRACT_REVISION=${OURBOX_PLATFORM_CONTRACT_REVISION}" \
  --artifact-metadata "OURBOX_PLATFORM_CONTRACT_VERSION=${OURBOX_PLATFORM_CONTRACT_VERSION}" \
  --artifact-metadata "OURBOX_PLATFORM_CONTRACT_CREATED=${OURBOX_PLATFORM_CONTRACT_CREATED}" \
  --input-metadata "PROFILE_DEFAULT=demo-apps" \
