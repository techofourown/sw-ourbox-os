...
APPLICATION_IMAGES_LOCK_SHA256=""
...
if [[ -f "${CONTRACT_DIR}/catalog.json" ]]; then
  APPLICATION_CATALOG_SHA256="$(sha256sum "${CONTRACT_DIR}/catalog.json" | awk '{print $1}')"
fi

if [[ -f "${CONTRACT_DIR}/images.lock.json" ]]; then
  APPLICATION_IMAGES_LOCK_SHA256="$(sha256sum "${CONTRACT_DIR}/images.lock.json" | awk '{print $1}')"
fi
...
OURBOX_APPLICATION_CATALOG_SHA256=${APPLICATION_CATALOG_SHA256}
OURBOX_APPLICATION_IMAGES_LOCK_SHA256=${APPLICATION_IMAGES_LOCK_SHA256}
EOF
